<#
.SYNOPSIS
    Complete SAIF Secure deployment including infrastructure and containers.
.DESCRIPTION
    This script deploys the complete SAIF Secure application to Azure including:
    1. Infrastructure deployment using Bicep (secure version with Entra ID)
    2. Container building and pushing to ACR (secure API)
    3. App Service configuration and restart
    4. Entra ID SQL Server configuration
.PARAMETER location
...
.EXAMPLE
    .\Deploy-SAIF-Secure.ps1 -location "swedencentral"
.EXAMPLE
    .\Deploy-SAIF-Secure.ps1 -location "germanywestcentral" -skipContainers
.NOTES
    Author: SAIF Team
    Version: 2.0.0
    Date: 2025-09-02
    Note: This is the secure version with Entra ID authentication
#>
...
if (-not $resourceGroupName) {
    if ($location -eq "swedencentral") {
        $resourceGroupName = "rg-saif-secure-swc01"
    } else {
        $resourceGroupName = "rg-saif-secure-gwc01"
    }
}
...
Show-Banner "SAIF Secure Deployment"
...
if (-not $PSBoundParameters.ContainsKey('location') -or -not $PSBoundParameters.ContainsKey('resourceGroupName')) {
    ...
    Write-Host "Configure your SAIF Secure deployment settings:" -ForegroundColor White
    Write-Host ""
    ...
    if (-not $PSBoundParameters.ContainsKey('resourceGroupName')) {
        $defaultRgName = if ($location -eq "swedencentral") { "rg-saif-secure-swc01" } else { "rg-saif-secure-gwc01" }
        Write-Host ""
...
# Get Entra ID admin configuration
Write-Host ""
Write-Host "🔐 Entra ID SQL Server Administrator Configuration" -ForegroundColor Cyan
Write-Host "The secure version requires an Entra ID user as SQL Server administrator." -ForegroundColor White
Write-Host ""

# Get current user as default suggestion
$currentUser = az ad signed-in-user show --query "{displayName:displayName, userPrincipalName:userPrincipalName, objectId:id}" -o json | ConvertFrom-Json
Write-Host "Current user (suggested default):" -ForegroundColor Yellow
Write-Host "  Name: $($currentUser.displayName)" -ForegroundColor White
Write-Host "  Email: $($currentUser.userPrincipalName)" -ForegroundColor White
Write-Host "  Object ID: $($currentUser.objectId)" -ForegroundColor Gray
Write-Host ""

$useCurrentUser = Read-Host "Use current user as SQL admin? (Y/n)"
if ($useCurrentUser -eq 'n' -or $useCurrentUser -eq 'N') {
    Write-Host ""
    $entraAdminEmail = Read-Host "Enter Entra ID admin email"
    
    # Try to look up user
    try {
        $adminUser = az ad user show --id $entraAdminEmail --query "{displayName:displayName, userPrincipalName:userPrincipalName, objectId:id}" -o json | ConvertFrom-Json
        $entraAdminObjectId = $adminUser.objectId
        Write-Host "✅ Found user: $($adminUser.displayName)" -ForegroundColor Green
    } catch {
        Write-Host "⚠️  Could not find user automatically" -ForegroundColor Yellow
        $entraAdminObjectId = Read-Host "Enter Entra ID admin Object ID"
    }
} else {
    $entraAdminEmail = $currentUser.userPrincipalName
    $entraAdminObjectId = $currentUser.objectId
}
...
Write-Host "  SQL Admin: $entraAdminEmail" -ForegroundColor White
...
Show-Banner "Step 1: Secure Infrastructure Deployment"
...
# Generate a secure password for SQL Server (fallback, since we use Entra ID only)
$sqlPassword = -join ((65..90) + (97..122) + (48..57) + 33,35,36,37,38,42,43,45,61,63,64 | Get-Random -Count 16 | ForEach-Object {[char]$_})

# Deploy infrastructure
Write-Host "🚀 Deploying secure infrastructure..." -ForegroundColor Green
$deploymentName = "main-secure-$(Get-Date -Format 'yyyyMMdd-HHmmss')"
$deploymentResult = az deployment group create `
    --resource-group $resourceGroupName `
    --template-file "../infra/main-secure.bicep" `
    --parameters location=$location sqlAdminPassword=$sqlPassword entraAdminEmail=$entraAdminEmail entraAdminObjectId=$entraAdminObjectId entraIdOnlyAuth=true `
    --name $deploymentName `
...
Write-Host "✅ Secure infrastructure deployed successfully!" -ForegroundColor Green
...
if (-not $skipContainers) {
    Show-Banner "Step 2: Secure Container Build & Deployment"
    
    Write-Host "🔨 Building and pushing secure API container..." -ForegroundColor Green
    az acr build --registry $acrName --image saif/api-secure:latest ../api-entra
    
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ Secure API container built and pushed" -ForegroundColor Green
    } else {
        Write-Host "❌ Secure API container build failed" -ForegroundColor Red
        exit 1
    }
...
# Final Summary
Show-Banner "🎉 SAIF Secure Deployment Complete!"
...
Write-Host "  ✅ Entra ID Managed Identity authentication" -ForegroundColor Green
Write-Host "  ✅ Parameterized SQL queries (no SQL injection)" -ForegroundColor Green
Write-Host "  ✅ Input validation and SSRF protection" -ForegroundColor Green
Write-Host "  ✅ TLS 1.2 encryption for all connections" -ForegroundColor Green
Write-Host ""
Write-Host "🔗 Test your deployment by visiting the URLs above" -ForegroundColor Cyan
Write-Host "🔄 To update containers only, use: .\Update-SAIF-Containers.ps1 -ResourceGroupName $resourceGroupName" -ForegroundColor Cyan
